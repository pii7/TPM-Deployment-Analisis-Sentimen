...
global_df_clean = None

# ---------------------------
# CONFIG (ubah sesuai kebutuhan)
# ---------------------------
# Tentukan path CSV secara robust:
# - Prioritaskan folder tempat script berada
# - Cek juga `os.getcwd()` dan beberapa lokasi umum
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
possible_paths = [
    os.path.join(SCRIPT_DIR, "data_gabungan.csv"),
    os.path.join(os.getcwd(), "data_gabungan.csv"),
    os.path.abspath(os.path.join(SCRIPT_DIR, "..", "data_gabungan.csv")),
    os.path.abspath(os.path.join(SCRIPT_DIR, "data", "data_gabungan.csv"))
]
FILEPATH = None
for p in possible_paths:
    if os.path.exists(p):
        ...
        break
if FILEPATH is None:
    # default (jelas dan mudah untuk debug) — gunakan path di folder script
    FILEPATH = os.path.abspath(os.path.join(SCRIPT_DIR, "data_gabungan.csv"))
...
print(f"[CONFIG] Using data file: {FILEPATH}")
...
# ---------------------------
# WORDCLOUD EXPORT (BYTES + FILE)
# ---------------------------
import io as _wc_io  # local alias to avoid shadowing
...
def generate_wordcloud_file(df, text_col="review_text_clean",
                            output_path="images/wordcloud_all.png"):
    """Generate WordCloud dan simpan file PNG."""
...
def train_svm_binary(df: pd.DataFrame,
    ...
    pipe = Pipeline([
        # [FIX NGRAM] SVM sudah menggunakan N-gram (1,2)
...
def build_features(texts, kind="count", use_svd=False, n_components=300, fit=None):
    if kind == "count":
        # CountVectorizer tetap unigram untuk perbandingan awal
        vec = CountVectorizer(analyzer="word", ngram_range=(1,1), lowercase=True, token_pattern=r"\b\w+\b")
    elif kind == "tfidf":
        # [FIX NGRAM] TfidfVectorizer di eksperimen LR diubah menjadi N-gram (1,2)
        vec = TfidfVectorizer(analyzer="word", ngram_range=(1,2), lowercase=True, token_pattern=r"\b\w+\b",
...
def run_logistic_regression_variants(df: pd.DataFrame,
    ...
    # Perubahan di build_features (N-gram (1,2) sekarang digunakan untuk TF-IDF)
    run_lr_binary(kind="tfidf", use_svd=False, name="TF-IDF N-gram(1,2) (tanpa DR)")
    run_lr_binary(kind="tfidf", use_svd=True, n_components=400, name="TF-IDF N-gram(1,2) + SVD")
    ...
    # Perubahan di build_features (N-gram (1,2) sekarang digunakan untuk TF-IDF)
    run_lr_multiclass(kind="tfidf", use_svd=False, name="TF-IDF N-gram(1,2) (tanpa DR)")
    run_lr_multiclass(kind="tfidf", use_svd=True, n_components=400, name="TF-IDF N-gram(1,2) + SVD")

# ---------------------------
# NAIVE BAYES
# ---------------------------
def run_naive_bayes(df: pd.DataFrame, text_col: str = "review_text_clean"):
    # [FIX NGRAM] TfidfVectorizer di eksperimen NB diubah menjadi N-gram (1,2)
    vec = TfidfVectorizer(analyzer="word", ngram_range=(1,2), token_pattern=r"\b\w+\b", lowercase=True, sublinear_tf=True, norm="l2")
    ...
    def run_nb_binary(clf, name):
        print(f"\n--- NB BINER | {name} ---")
        pipe = Pipeline([("tfidf", vec), ("nb", clf)]) # vec sekarang N-gram (1,2)
    ...
    run_nb_binary(MultinomialNB(alpha=1.0), "MultinomialNB (N-gram 1,2)")
    run_nb_binary(ComplementNB(alpha=1.0), "ComplementNB (N-gram 1,2)")
    ...
    run_nb_3class(MultinomialNB(alpha=1.0), "MultinomialNB (N-gram 1,2)")
    run_nb_3class(ComplementNB(alpha=1.0), "ComplementNB (N-gram 1,2)")
...
def compare_models_visual(df: pd.DataFrame, text_col: str = "review_text_clean", label_col: str = "sentiment_binary"):
    ...
    # [FIX NGRAM] TfidfVectorizer untuk LR dan NB diubah menjadi N-gram (1,2)
    pipelines = {
        ...
        "Logistic Regression": Pipeline([("tfidf", TfidfVectorizer(analyzer="word", ngram_range=(1,2), token_pattern=r"\b\w+\b", lowercase=True, sublinear_tf=True, norm="l2")), ("clf", LogisticRegression(solver="lbfgs", max_iter=10000, n_jobs=-1))]), # <-- Diubah
        "Naive Bayes (Complement)": Pipeline([("tfidf", TfidfVectorizer(analyzer="word", ngram_range=(1,2), token_pattern=r"\b\w+\b", lowercase=True, sublinear_tf=True, norm="l2")), ("clf", ComplementNB(alpha=1.0))]) # <-- Diubah
    }
...
def train_final_pipelines_and_live_inference(df: pd.DataFrame, text_col: str = "review_text_clean"):
    ...
    def make_vec_svm():
        # Vektorizer SVM (sudah N-gram 1,2)
        return TfidfVectorizer(analyzer="word", ngram_range=(1,2), min_df=2, max_df=0.98, sublinear_tf=True, norm="l2")

    # [FIX NGRAM] Mengubah make_vec_unigram menjadi N-gram (1,2)
    def make_vec_ngram_1_2():
        # Vektorizer baru untuk LR/NB, sekarang menggunakan N-gram (1,2)
        return TfidfVectorizer(analyzer="word", ngram_range=(1,2), token_pattern=r"\b\w+\b", lowercase=True, sublinear_tf=True, norm="l2")

    _svm_base = LinearSVC(C=0.5, tol=1e-4)
    # SVM: make_vec_svm() -> N-gram (1,2)
    ...
    # LR: make_vec_ngram_1_2() -> N-gram (1,2)
    lr_bin = make_pipeline(make_vec_ngram_1_2(), LogisticRegression(solver="lbfgs", max_iter=10000, n_jobs=-1))
    lr_bin.fit(_df_bin[text_col].astype(str).values, _df_bin["sentiment_binary"].astype(str).values)

    # NB: make_vec_ngram_1_2() -> N-gram (1,2)
    nb_bin = make_pipeline(make_vec_ngram_1_2(), ComplementNB(alpha=1.0))
    ...
    sample_texts = [
        # Menambahkan contoh teks yang sangat pendek untuk tes
        "enak",
        "tidak enak",
        "kecewa",
    ...
    def predict_all_models_local(texts_list):
        rows = []
        for t in texts_list:
            # Clean text sebelum prediksi!
            t_clean = clean_text(t)

            # SVM
            try:
                # Perkiraan indeks untuk 'neg' dan 'pos'
                classes = svm_bin.classes_
                neg_idx = np.where(classes == 'neg')[0][0]
                pos_idx = np.where(classes == 'pos')[0][0]

                pred = svm_bin.predict([t_clean])[0]
                proba = svm_bin.predict_proba([t_clean])[0]
            except Exception:
                pred = svm_bin.predict([t_clean])[0]
                proba = [np.nan, np.nan] # fallback

            rows.append({
                "Model": "SVM (Linear) N-gram(1,2)",
                "Input": t,
                "Prediksi": pred,
                "p(neg)": float(proba[neg_idx]) if not np.isnan(proba[0]) else None,
                "p(pos)": float(proba[pos_idx]) if not np.isnan(proba[1]) else None
            })

            # LR
            classes = lr_bin.classes_
            neg_idx = np.where(classes == 'neg')[0][0]
            pos_idx = np.where(classes == 'pos')[0][0]
            pred = lr_bin.predict([t_clean])[0]
            proba = lr_bin.predict_proba([t_clean])[0]
            rows.append({
                "Model": "Logistic Regression N-gram(1,2)",
                "Input": t,
                "Prediksi": pred,
                "p(neg)": float(proba[neg_idx]),
                "p(pos)": float(proba[pos_idx])
            })

            # NB
            classes = nb_bin.classes_
            neg_idx = np.where(classes == 'neg')[0][0]
            pos_idx = np.where(classes == 'pos')[0][0]
            pred = nb_bin.predict([t_clean])[0]
            proba = nb_bin.predict_proba([t_clean])[0]
            rows.append({
                "Model": "Naive Bayes (Complement) N-gram(1,2)",
                "Input": t,
                "Prediksi": pred,
                "p(neg)": float(proba[neg_idx]),
                "p(pos)": float(proba[pos_idx])
...
def bundle_and_save_models(models: Dict[str, Any], out_path: str = None):
    ...
    # Attempt to generate WordCloud base64 (if df available)
    try:
        all_text = " ".join(global_df_clean["review_text_clean"].astype(str).tolist()) if global_df_clean is not None else ""
        wc_b64 = generate_wordcloud_base64(all_text) if all_text else None
    except Exception as e:
        print("Failed to generate WordCloud for bundle:", e)
        wc_b64 = None

    bundle_meta = {
        "task": "binary",
        "models": list(models.keys()),
        "has_wordcloud": wc_b64 is not None
    }

    models_bundle = {
        **models,
        "meta": bundle_meta,
        "wordcloud_png": wc_b64   # wordcloud included in all_models_bin.pkl
    }
...
def main():
    ...
    global global_df_clean
    global_df_clean = df_clean

    # Generate WordCloud ke file
    try:
        generate_wordcloud_file(df_clean, text_col="review_text_clean", output_path="images/wordcloud_all.png")
    except Exception as e:
        print("Gagal membuat WordCloud:", e)
